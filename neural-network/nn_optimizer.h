...
namespace utec::neural_network {
    ...
    class Adam final : public IOptimizer<T> {
        ...
        std::size_t t = 0;
    public:
        explicit Adam(T learning_rate = 0.001, T beta1 = 0.9, T beta2 = 0.999, T epsilon = 1e-8): learning_rate(learning_rate), beta1(beta1), beta2(beta2), epsilon(epsilon) {}
        ...
        void update(Tensor<T, 2>& params, const Tensor<T, 2>& grads) override {
            ...
            const Tensor<T, 2> m_hat = m / (T{1} - std::pow(beta1, t));
            const Tensor<T, 2> v_hat = v / (T{1} - std::pow(beta2, t));

            params -= m_hat * learning_rate / (std::apply(v_hat, [](const T x) { return std::sqrt(x); }) + epsilon);
        }
