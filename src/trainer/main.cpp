...
public:
    ...
    void train(NeuralNetwork<double>& net, std::mt19937& rng) const {
        ...
        net.train<CrossEntropyLoss>(inputs, outputs, 1000, samples.size(), 0.3, rng);
    }
...
namespace {
    ...
    auto load_neural_network(std::mt19937& rng) -> NeuralNetwork<double> {
        ...
        const auto he_init = [&](Tensor<double, 2>& tensor) {
        ...
        net.add_layer<Dense<double>>(IMAGE_LENGTH, 64, he_init, he_init);
