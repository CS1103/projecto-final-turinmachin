...
TEST_CASE("Dense + Sigmoid with batch size 2", "[neural]") {
    ...
    input(0, 0) = 0.5f; input(0, 1) = -0.3f;
    input(1, 0) = 1.0f; input(1, 1) =  0.2f;
...
TEST_CASE("neural network trains and reduces loss", "[neural][train]") {
    ...
    const auto he_init = [&](Tensor<float, 2>& tensor) {
        const float scale = std::sqrt(2.0 / static_cast<float>(tensor.shape()[1]));
    ...
    for (size_t i = 0; i < 4; ++i) {
        ...
        if (y(i, 0) == 1) { REQUIRE(output > 0.8f); }
        else { REQUIRE(output < 0.2f); }
    }
}
...
TEST_CASE("neural network save/load consistency", "[neural][save]") {
    ...
    for (size_t i = 0; i < out1.size(); ++i)
        REQUIRE(out1[i] == Approx(out2[i]).epsilon(0.001));
}
